      ...
      *    *** サムネイルimg データ
       ...
       01  WORK-AREA.
           03  WK-PGM-NAME     PIC  X(008) VALUE "TEST103 ".

           03  WK-PRM1-F-NAME  PIC  X(032) VALUE "TEST103.PRM1 ".
           03  WK-PIN1-F-NAME  PIC  X(064) VALUE "XXXXXXXX.PIN1".
      *     03  WK-PIN1-F-NAME  PIC  X(032) VALUE "TEST103.PIN1".
           03  WK-PIN2-F-NAME  PIC  X(064) VALUE "XXXXXXXX.PIN2".
                       ...
                       IF      TBL01-ITEM-LEN (K1) NOT = ZERO
                           ...
                           EVALUATE TRUE
                             WHEN TBL02-NO-IDX (K2) = ZERO
                               ...
                               MOVE    TBL01-NO (K1) TO  TBL02-NO (K2 1)
                               MOVE    "Y"         TO      SW-YES
                               ADD     1           TO
                                       TBL01-CNT (TBL01-NO (K1))
                             WHEN TBL02-NO-IDX (K2) = 1
                               IF      TBL02-NO (K2 1) =   TBL01-NO (K1)
                                       CONTINUE
                               ELSE
                                   MOVE    2        TO TBL02-NO-IDX (K2)
                                   MOVE TBL01-NO (K1) TO TBL02-NO (K2 2)
                                   ADD     1           TO
                                           TBL01-CNT (TBL01-NO (K1))
                               END-IF
                             WHEN TBL02-NO-IDX (K2) = 2
                               IF      TBL02-NO (K2 1) =   TBL01-NO (K1)
                                    OR TBL02-NO (K2 2) =   TBL01-NO (K1)
                                       CONTINUE
                               ELSE
                                   MOVE    3        TO TBL02-NO-IDX (K2)
                                   MOVE TBL01-NO (K1) TO TBL02-NO (K2 3)
                                   ADD     1           TO
                                           TBL01-CNT (TBL01-NO (K1))
                               END-IF
                             WHEN TBL02-NO-IDX (K2) = 3
                               IF      TBL02-NO (K2 1) =   TBL01-NO (K1)
                                    OR TBL02-NO (K2 2) =   TBL01-NO (K1)
                                    OR TBL02-NO (K2 3) =   TBL01-NO (K1)
                                       CONTINUE
                               ELSE
                                   MOVE    4        TO TBL02-NO-IDX (K2)
                                   MOVE TBL01-NO (K1) TO TBL02-NO (K2 4)
                                   ADD     1           TO
                                           TBL01-CNT (TBL01-NO (K1))
                               END-IF
                             WHEN TBL02-NO-IDX (K2) = 4
                               IF      TBL02-NO (K2 1) =   TBL01-NO (K1)
                                    OR TBL02-NO (K2 2) =   TBL01-NO (K1)
                                    OR TBL02-NO (K2 3) =   TBL01-NO (K1)
                                    OR TBL02-NO (K2 4) =   TBL01-NO (K1)
                                       CONTINUE
                               ELSE
                                   MOVE    5        TO TBL02-NO-IDX (K2)
                                   MOVE TBL01-NO (K1) TO TBL02-NO (K2 5)
                                   ADD     1           TO
                                           TBL01-CNT (TBL01-NO (K1))
                               END-IF
                           END-EVALUATE
      ...
      *    *** テーブルヒットしない時、テーブルの最後、その他にする
           IF      SW-YES      =       "N"
                   MOVE    1           TO      TBL02-NO-IDX (K2)
                   MOVE    K1-MAX      TO      TBL02-NO (K2 1)
                   ADD     1           TO      TBL01-CNT(K1-MAX)
           END-IF

           MOVE    WK-REC      TO      TBL02-POT1-REC (K2)
           MOVE    K2          TO      K2-MAX
           .
       S100-EX.
           EXIT.
      ...
      *    *** TBL02 => WRITE POT1
       S120-10.
           ...
           PERFORM VARYING K3 FROM 1 BY 1
                   UNTIL K3 > TBL02-NO-IDX (K2)
                   IF      K1          =       TBL02-NO (K2 K3)
                           WRITE   POT1-REC    FROM
                                               TBL02-POT1-REC (K2)
                           ADD     1           TO      WK-POT1-CNT
                   END-IF
